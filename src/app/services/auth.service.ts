...
export class AuthService {
  ...
  private baseUrl =`${environment.urlServer}/accounts`;
  ...
  signup(account: AccountData): Observable<boolean> {
    const hashedPassword = this.hashPassword(account.password);
    const userWithHashedPassword = { ...account, password: hashedPassword };
    ...
    return this.http.post<AccountData>(this.baseUrl, userWithHashedPassword).pipe(
      ...
      catchError((error) => {
        ...
        return of(false); // Si hay error, retorna false
      })
    );
  }
